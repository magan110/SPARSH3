  ...
  void _validateToken(String value) {
    ...
    if (_maxAttemptsReachedTokens.contains(cleanValue)) {
      ...
      if (lastErrorTime == null ||
          now.difference(lastErrorTime) > const Duration(seconds: 5)) {
              ...
              children: [
                Table(
                  ...
                  children: [
                    ...
                    _buildTableRow(
                      'Expired Scan',
                      expiredScan.toString(),
                      Colors.orange,
                    ),
                    _buildTableRow(
                      'Already Scanned',
                      alreadyScanned.toString(),
                      Colors.deepPurple,
                    ),
                    _buildTableRow(
                      'Invalid Scan',
                      invalidScan.toString(),
                      Colors.red,
                    ),
                    _buildTableRow(
                      'Total Amount',
                      ...
                      onPressed: () async {
                        ...
                        try {
                          ...
                          for (var tokenData in allTokenData) {
                            await tokenDatabase.insertToken(
                              tokenData['token'],
                              tokenData,
                            );
                            successCount++;
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                '$successCount tokens saved successfully',
                              ),
                          ...
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => const AllTokens(),
                            ),
                          );
                        } catch (e) {
                          // Show error message
    ...
    return Scaffold(
      body: Padding(
        ...
        child: Column(
          children: [
            SizedBox(
              height: (screenHeight * 0.35).clamp(250.0, 350.0),
              child: Stack(
                children: [
                  Container(
                    decoration: BoxDecoration(

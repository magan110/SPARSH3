  ...
  void _onSearchChanged() {
    ...
    setState(() {
      filteredRows =
          allRows
              .where(
                (row) => row.any((cell) => cell.toLowerCase().contains(search)),
              )
              .toList();
    });
  }
          ...
          child: Column(
            ...
            children: [
              _buildTextField(
                ...
                onTap: () => _pickDate(context, asOnDate),
              ),
              _buildDropdown(
                label: 'Report As Per (Aman / R B Gupta)',
                value: reportAsPerValue,
                items: reportAsPerList,
                onChanged: (val) => setState(() => reportAsPerValue = val!),
              ),
              _buildDropdown(
                label: 'Active / Left',
                value: activeLeftValue,
                items: activeLeftList,
                onChanged: (val) => setState(() => activeLeftValue = val!),
              ),
              _buildTextField(
                controller: leftFromDate,
                label: 'Left From Date',
                icon: Icons.calendar_today,
                onTap: () => _pickDate(context, leftFromDate),
              ),
              _buildTextField(
                controller: leftToDate,
                label: 'Left To Date',
                icon: Icons.calendar_today,
                onTap: () => _pickDate(context, leftToDate),
              ),
              _buildDropdown(
                label: 'Site/Mktg/Grc/Katni/All',
                value: siteValue,
                items: siteList,
                onChanged: (val) => setState(() => siteValue = val!),
              ),
              _buildDropdown(
                label: 'Comm/Hr/Mktg/Tech/All',
                value: commHrValue,
                items: commHrList,
                onChanged: (val) => setState(() => commHrValue = val!),
              ),
              _buildDropdown(
                label: 'Zone Wise/All',
                value: zoneValue,
                items: zoneList,
                onChanged: (val) => setState(() => zoneValue = val!),
              ),
              _buildDropdown(
                label: 'Sorting Order',
                value: sortingOrderValue,
                items: sortingOrderList,
                onChanged: (val) => setState(() => sortingOrderValue = val!),
              ),
                ...
                children: [
                  ElevatedButton(
                    style: ElevatedButton.styleFrom(
                      ...
                      minimumSize: Size(
                        (MediaQuery.of(context).size.width * 0.22).clamp(
                          80.0,
                          100.0,
                        ),
                        (MediaQuery.of(context).size.width * 0.115).clamp(
                          40.0,
                          50.0,
                        ),
                      ),
              ...
              Card(
                elevation: 13,
                shadowColor: Colors.blue.shade100.withValues(alpha: 0.38),
